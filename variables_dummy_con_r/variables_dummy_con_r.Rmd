...
Los datos categóricos o nominales, como su nombre lo indica, son usados para nombrar o categorizar información. Este tipo de dato se caracteríza por no ser ordenado, incluso si se usan números para representarlos.

El nombre de las diferentes razas de perros es un dato categórico. Aunque puedes ordenar todos los nombres alfabéticamente, carece de sentido conceptual que "Akita" se encuentre antes que "Beagle" o que "Collie" se encuentre despúes de "Chihuahua". Esto no nos informa ni nos ayuda a entender sobre las características de los perros que pertenecen a estas razas.
...
Si desamos realizar una regresión linear con la función `lm()` de *stats*, no tendremos problemas para realizar correctamente sus operaciones. 

Sin embargo, este no siempre es el caso con funciones incluídas en paquetes de terceros o que importan procedimientos desde otros lenguajes de programación.
...
# Las variables dummy
Crear variables dummy implica transformar datos de un formato "alto", en el que cada columna contiene la infomación de una variable, a datos con un formato "ancho", en los que múltiples columnas contienen la información de las dos variables, codificada de manera binaria, esto es, con 0 y 1.
...
En este ejemplo, la manera de interpretar las variables dummie es observando las columnas con el prefijo "raza_" y "vac". 
...
De este modo, el primer perro en nuestro datos es un akita, pues tiene un 1 en la columna *raza_akita* y el tipo de vacuna que se le ha aplicado es bet, pues tiene un 1 en *vac_beta*.
...
Agregamos el prefijo "raza" y "vac" a los datos de la columnas con esos nombres. Esto nos servira más adelante organizar y distinguir nuestras variables dummy.
...
El resultado anterior está bien si queremos tranformar en dummy una variable a la vez,  pero es poco práctica para realizar múltiples transformaciones.
...
Usamos `reduce` de *purrr* e `inner_join` de *dplyr* para unir estas dos data frames en una sola, tomando la columna *id* como índice, un proceso similar lo que haríamos en SQL.
```{r map_reduce}
map(c("raza", "vac"), columna_dummy, df = perros) %>% 
...
Problemos nuestra nueva función.
